"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.FilesService = void 0;
const common_1 = require("@nestjs/common");
const sequelize_1 = require("@nestjs/sequelize");
const file_entity_1 = require("./entities/file.entity");
const jobs_service_1 = require("../jobs/jobs.service");
let FilesService = class FilesService {
    constructor(fileModel, jobsService) {
        this.fileModel = fileModel;
        this.jobsService = jobsService;
    }
    async uploadFile(userId, file, metadata) {
        if (!file) {
            throw new common_1.BadRequestException('File is required');
        }
        const fileEntity = this.fileModel.build({
            userId,
            originalFilename: file.originalname,
            storagePath: file.path,
            title: metadata.title,
            description: metadata.description,
            status: 'uploaded',
        });
        const savedFile = await fileEntity.save();
        await this.jobsService.addFileProcessingJob(savedFile.id);
        return { fileId: savedFile.id, status: 'uploaded' };
    }
    async getFile(userId, fileId) {
        const file = await this.fileModel.findOne({ where: { id: fileId, userId } });
        if (!file) {
            throw new common_1.ForbiddenException('File not found or access denied');
        }
        return file;
    }
    async getFiles(userId, page, limit) {
        const offset = (page - 1) * limit;
        const { rows: data, count: total } = await this.fileModel.findAndCountAll({
            where: { userId },
            limit,
            offset,
        });
        return { data, meta: { page, limit, total } };
    }
    async getFileById(fileId) {
        return this.fileModel.findByPk(fileId);
    }
    async updateFileStatus(fileId, status, extractedData) {
        await this.fileModel.update({ status, extractedData }, { where: { id: fileId } });
    }
};
exports.FilesService = FilesService;
exports.FilesService = FilesService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, sequelize_1.InjectModel)(file_entity_1.File)),
    __metadata("design:paramtypes", [Object, jobs_service_1.JobsService])
], FilesService);
//# sourceMappingURL=files.service.js.map